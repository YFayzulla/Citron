                    ...
                    <table class="w-full whitespace-no-wrap hover:table-fixed">
                        ...
                        @foreach($services as $service)
                            <tr class="text-gray-700 dark:text-gray-400">
                                ...
                                <td class="px-4 py-3 text-sm">{{$service->name_uz}}</td>
                                <td class="px-4 py-3 text-sm">{{$service->desc_uz}}</td>
                                <td class="px-4 py-3 text-sm">
                                    <img src="Aphoto/{{$service->image}}" width="80px">
                                </td>
                                <td class="px-4 py-3">
                                    <div class="flex items-center space-x-4 text-sm">
                                        ...
                                        <form action="{{route('services.destroy',$service->id)}}" method="post" id="form-delete">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button"
                                                    class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
                                                    onclick="delete_button({{$service->id}})">
                                                <svg class="w-5 h-5" aria-hidden="true" fill="currentColor"
                                                     viewBox="0 0 20 20">
